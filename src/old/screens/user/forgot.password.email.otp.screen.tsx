...
const ForgotPasswordOTPScreen = memo((navigationRoute: any) => {
  ...
  const handleRestore = async (value: ForgotPasswordMutationVariables) => {
    ...
    post(site_ajax_url("otp.php?a=otpviaphone"),{code:value.txtForgotPassword},(res:any) =>{
       setLoading(false);
       if(res && res?.code==1){
          if(res.message){
            Toast.show({
          ...
          navigation.navigate(AppRoutes.FORGOT_PASSWORD_OTP_NEW,{phone:route?.params?.phone,code:value.txtForgotPassword});
       }else{ 
          Toast.show({
            type: "error",
            text2: res?.error,
